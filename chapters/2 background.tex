...
\todo{Might have to remove this footnote depending on Motivation chapter}
...
For instance, the \texttt{bert-base-uncased} tokenizer used in this work splits the word \textit{enmity} into the sub-word tokens \textit{en} and \textit{\#\#mity} (sub-word tokens which are not the start of a word begin with \textit{\#\#} in their text representation for BERT tokenizers).
