...
\todo{Say on which corpora the above approach is performed, and how the results are aggregated. Describe train/test splitting}
...
\contentdescription{
	Pure Description of results without deep interpretations
}
...
Single Token Ablation with the Sentence Embeddings achieves the highest results across corpus types.
...
For the transformer attention approach, both the model for Next Sentence Prediction and the model for Sentence Embeddings produce very similar results.
...
\todo{Show speed/runtime of approaches}
