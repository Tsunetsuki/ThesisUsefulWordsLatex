...
The primary metric for evaluating a vocabulary list's efficiency will be the approach explained in chapter \ref{sec:list-generation}: 
...
The increase of vocabulary size is exponential, as this will ensure a quick evaluation and performance is expected to not grow very much towards the end of the list, where the less useful words should be (this assumption can be confirmed in the evaluation results in chapter \ref{sec:results}).
...
This chapter has introduces some pre-existing lists as points of comparison for our generated vocabulary lists.
...
The next chapter will introduce some of these methods, which we will use to generate comparison lists to our own.
